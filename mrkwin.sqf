...
if (_marcador in aeropuertos) then
  ...
  _nul = [20,10] remoteExec ["prestige",2];
...
if (_marcador in power) then
  ...
  //hint "You have cleared this Power Plant\nAAF Prestige in the nearest city has decreased";
...
if (_marcador in puertos) then
  {
  //_nul = [-10,0,_posicion] remoteExec ["citySupportChange",2];
  [["TaskSucceeded", ["", "Seaport Taken"]],"BIS_fnc_showNotification"] call BIS_fnc_MP;
  _nul = [10,10] remoteExec ["prestige",2];
