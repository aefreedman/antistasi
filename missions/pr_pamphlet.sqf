...
_tsk = ["PR",[side_blue,civilian],[format ["Supporters of our cause have been working hard to bring the population of %1 over to our side. Help these brave volunteers by delivering these leaflets to safe drop locations. Pick up will take place at %2:%3, so you better have them in place by then. This might be easier if you manage to remain undetected.",_targetName,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"Leaflet Drop",_targetMarker],_targetPosition,"CREATED",5,true,true,"Heal"] call BIS_fnc_setTask;
...
_crate3 = "Land_WoodenCrate_01_F" createVehicle position player;
_crate3 attachTo [PRTruck,[0,-2.5,-0.25]];
_crate3 setDir (getDir PRTruck + 78);
_crates pushBack _crate3;

_crate2 = "Land_WoodenCrate_01_F" createVehicle position player;
_crate2 attachTo [PRTruck,[0.4,-1.1,-0.25]];
_crate2 setDir (getDir PRTruck);
_crates pushBack _crate2;

_crate1 = "Land_WoodenCrate_01_F" createVehicle position player;
_crate1 attachTo [PRTruck,[-0.4,-1.1,-0.25]];
_crate1 setDir (getDir PRTruck);
_crates pushBack _crate1;
...
_currentGeneratorCount: number of sites done
_currentGenerator: current site
