...
if (_tipo == "PR") then {
	...
	if (_pickTarget) then {
		...
		*/
		...
		if (!visibleMap) exitWith {_noMission = true};

		_posicionTel =+ posicionTel;

		_sitio = [ciudades, _posicionTel] call BIS_Fnc_nearestPosition;

		if (count _output == 0) exitWith {_noMission = true};
		_sitio = _output select 0;
		if !((_sitio in _posiblesA) || (_sitio in _posiblesB)) exitWith {_noMission = true};

		if (_sitio in _posiblesA) then {
