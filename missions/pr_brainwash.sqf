...
_tsk = ["PR",[side_blue,civilian],[format ["We have noticed that the town of %1 is sympathetic to our cause. Move their with the truck and establish a semi-permanent propaganda station in order to spread the word. Once established, stay with and guard the station for as long as you deem necessary to raise town support. It would be wise to expect a number of enemy military responses to this action. You have until %2:%3 to arrive and set it up.",_targetName,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"Establish Propaganda Station",_targetMarker],_targetPosition,"CREATED",5,true,true,"Heal"] call BIS_fnc_setTask;
...
/*
_graf5 = "Land_Graffiti_01_F" createVehicle _pos;
_graf5 attachTo [propTruck,[0,0.5,1.05]];
_graf5 setDir (getDir propTruck);
_graf5 setVectorUp [0,0.99,0.01];
_grafArray pushBack _graf5;
*/
...
_tsk = ["PR",[side_blue,civilian],[format ["Protect the station for as long as you can. We have enough battery capacity to run this station until %2:%3, so don't drag this out.",_targetName,numberToDate [2035,_fechalimnum] select 3,numberToDate [2035,_fechalimnum] select 4],"Establish Propaganda Station",_targetMarker],propTruck,"ASSIGNED",5,true,true,"Heal"] call BIS_fnc_setTask;
