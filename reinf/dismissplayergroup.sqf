...
if (_ai) then
  ...
  if (!isMultiplayer) then {_nul = [_hr,_resourcesFIA/2] remoteExec ["resourcesFIA",2];} else {_nul = [_hr,0] remoteExec ["resourcesFIA",2]; [_resourcesFIA/2] call resourcesPlayer};
