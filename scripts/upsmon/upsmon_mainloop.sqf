      ...
      case "FIREMISSION":
      {
        If (count _attackpos > 0 || count (_grp getvariable ["UPSMON_Artifiremission",[]]) > 0) then
        {
          If (_grp getvariable ["UPSMON_OnBattery",false]) then
          {
            ...
            If ((!(_grp getvariable ["UPSMON_Batteryfire",false])) and ({((side _x == civilian) and (_x distance _attackpos < (200))) or ((side _x == side (leader _grp)) and (_x distance _attackPos < 100))} count allUnits == 0)/* and (_area > 5)*/) then //modified by Barbolani for Antistasi
            {
              _artitarget = _attackpos;
              _firemission = "HE";
              _roundsask = 1;
