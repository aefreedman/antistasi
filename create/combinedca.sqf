...
private ["_posorigen","_tipogrupo","_nombreorig","_markTsk","_wp1","_soldados","_landpos","_pad","_vehiculos","_wp0","_wp3","_wp4","_wp2",
		...
		"_tiempo","_solMax","_nul","_coste","_tipo","_threatEvalAir","_threatEvalLand","_pos","_timeOut","_plane","_planesAAF"];
...
if (_CSAT) then
	...
	if (_resourcesAAF > 20000) then
		...
		{
		_nul = [5,-20] remoteExec ["prestige",2]
		};
	...
	for "_i" from 1 to _cuenta do
		...
		if (_i == _cuenta) then
			...
			{
			_tipoVeh = selectRandom planesCSAT;
			};
		_timeOut = 0;
	...
	[_mrkDestino] spawn
		...
		for "_i" from 0 to round (random 2) do
			{
			_nul = [_mrkdestino, selectRandom opCASFW] spawn airstrike;
			sleep 30;
			};
...
if (_base != "") then
	...
	for "_i" from 1 to _nveh do
		...
		if !(_tipoVeh in vehTank) then {
			if (_tipoVeh in vehIFV) then {_tipogrupo = infTeam call BIS_fnc_selectRandom} else {_tipogrupo = infSquad call BIS_fnc_selectRandom};
			_grupo = [_posorigen, side_green, (cfgInf >> _tipogrupo)] call BIS_Fnc_spawnGroup;
			//{[_x] spawn genInit;[_x] join _grupoVeh;_x assignAsCargo _veh;_x moveInCargo _veh; _soldados pushBack _x;} forEach units _grupo;
...
if (_aeropuerto != "") then
	{
	[_aeropuerto,60] call addTimeForIdle;
...
if (not(_mrkDestino in mrkAAF)) then
	...
	if (!_CSAT) then
		{
		_tsk = ["AtaqueAAF",[side_blue,civilian],[format ["AAF Is attacking from the %1. Intercept them or we may loose a sector",_nombreorig],"AAF Attack",_markTsk],getMarkerPos _markTsk,"SUCCEEDED",10,true,true,"Defend"] call BIS_fnc_setTask;
		}
	else
		{
	...
	if (!_CSAT) then
		{
		_tsk = ["AtaqueAAF",[side_blue,civilian],[format ["AAF Is attacking from the %1. Intercept them or we may loose a sector",_nombreorig],"AAF Attack",_markTsk],getMarkerPos _markTsk,"FAILED",10,true,true,"Defend"] call BIS_fnc_setTask;
