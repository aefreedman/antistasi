...
if ((_veh isKindOf "ReammoBox_F") or (_veh isKindOf "Box_East_WpsLaunch_F")) exitWith {[_veh] call cajaAAF};
...
if ((_tipo in vehTrucks) or (_tipo in vehPatrol) or (_tipo in vehSupply) or (_tipo in vehAAFAT) or (_tipo == "I_Boat_Armed_01_minigun_F")) then
  ...
  if !(_tipo in vehAAFAT) then
    ...
    _veh addEventHandler ["killed",{[-1000] remoteExec ["resourcesAAF",2]}];
    ...
    if ((_tipo in vehAPC) or (_tipo in vehIFV)) then
      {
      _veh addEventHandler ["killed",{if (side (_this select 1) == side_blue) then {[_this select 0] call AAFassets;[-2,2,position (_this select 0)] remoteExec ["citySupportChange",2]}}];
      ...
      if (_tipo in vehTank) then
        {
        _veh addEventHandler ["killed",{if (side (_this select 1) == side_blue) then {[_this select 0] call AAFassets; call AAFassets;[-5,5,position (_this select 0)] remoteExec ["citySupportChange",2]}}];
        _veh addEventHandler ["HandleDamage",{private ["_veh"]; _veh = _this select 0; if (!canFire _veh) then {[_veh] call smokeCoverAuto}}];
        };
      };
    };
  ...
  if (_tipo in planesAAF) then
    ...
    if (_tipo in heli_unarmed) then
      {
      _veh addEventHandler ["killed",{[-4000] remoteExec ["resourcesAAF",2]}];
      }
    else
      {
