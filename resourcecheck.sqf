...
// Do not tick until Switch Commander dialog has been set (see initmenu)
...
while {true} do
  ...
  // STEP: Determine support changes for each city
  {
    _ciudad = _x;
    ...
    if ((_prestigeOPFOR < _prestigeBLUFOR) and (_ciudad in mrkAAF)) then
    {
      ...
      _nul = [0,5] remoteExec ["prestige",2];
  ...
  // STEP: For each factory, check power. If powered, increase multiplier provided.
  {
    _fabrica = _x;
  ...
  // STEP: For each Resource, check power. If powered, add to income.
  {
    _recurso = _x;
...
/*
  ...
  if (count _fb > 5) then {
    ...
    if ((count _fb == 5) && (skillAAF < 10)) then {
      skillAAF = 10;
      maxSkillAAF = 13;
    }
    else {
      if ((count _fb == 4) && (skillAAF < 6)) then {
        skillAAF = 8;
        maxSkillAAF = 10;
      }
      else {
        if ((count _fb == 3) && (skillAAF < 3)) then {
          skillAAF = 6;
          maxSkillAAF = 8;
        }
        else {
          if ((count _fb == 2) && (skillAAF < 3)) then {
            skillAAF = 4;
            maxSkillAAF = 6;
          }
          else {
            if ((count _fb == 1) && (skillAAF < 3)) then {
              skillAAF = 2;
              maxSkillAAF = 4;
            };
          };
  ...
  */
  ...
  // STEP: Actually set HR/resources
  ...
  // STEP: Make sure we have a player commander assigned.
  if (isMultiplayer) then {[] spawn assignStavros};

  // STEP: Create random mission
  if (isMultiplayer and (not("AtaqueAAF" in misiones))) then
  {
    if (random 100 < 50) then {[] call missionRequestAUTO};
  }
  else {
    if (random 200 < 50) then {[] call missionRequestAUTO};
  };

  // STEP: Spawn road patrols
  if (AAFpatrols < 3) then {[] remoteExec ["genRoadPatrol",hcAttack]};

  // STEP: Some kind of static check?
  {
    _veh = _x;
